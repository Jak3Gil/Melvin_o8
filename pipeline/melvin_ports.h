...
typedef struct {
    ...
    /* Production optimization: Reusable buffers (zero allocations per frame) */
    uint8_t *serialize_buffer;           /* Buffer for serializing input frames */
    size_t serialize_buffer_capacity;    /* Current capacity of serialize buffer */
    uint8_t *output_buffer;              /* Buffer for reading output from .m file */
    size_t output_buffer_capacity;       /* Current capacity of output buffer */
    
    /* Production optimization: Output port cache (O(1) lookup instead of O(n)) */
    MelvinPort *output_port_cache[256];  /* Maps port_id -> port* for output ports */
} MelvinPortManager;
